...
def filter_corporate_bond_csas(config, selected_cps=None):
    ...
    config.CSA_FUNDS_MAPPED = csa_funds_mapped[csa_funds_mapped['CSA_ID'].isin(corp_csa_ids)]
    config.CSA_VALUATIONS = csa_valuations[csa_valuations['CSA_ID'].isin(corp_csa_ids)]
...
def build_counterparty_filter(config):
    ...
    config = filter_corporate_bond_csas(config, ss.selected_cps)
