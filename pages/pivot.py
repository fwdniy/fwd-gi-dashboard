...
def build_filters():
    ...
    with st.expander("Filters"):
        ...
        filter.build_multiselect('Columns', columns, columns)
        #filter.build_vanilla_tree('Values', values, values, checked=False)
        filter.build_multiselect('Values', values, values)
...
def query_data():
    ...
    columns = st.session_state['tree_selected_columns']
    values = st.session_state['tree_selected_values']
    current_date = st.session_state['Valuation Date']
    comparison_date = st.session_state['Comparison Date']
    ...
    query = f'SELECT CLOSING_DATE, {", ".join(columns)}, {", ".join(values)} FROM funnel.funnelweb WHERE fund_code IN ({fund_codes}) AND closing_date IN (\'{current_date}\', \'{comparison_date}\');'
    ...
    gb.configure_column('CLOSING_DATE', pivot=True)
