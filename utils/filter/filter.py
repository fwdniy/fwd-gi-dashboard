...
def build_date_filter_buttons(label: str, dates: list[datetime], default: datetime = None, key: str = 'selected_date', date: datetime = None, pill_dates: list[datetime] = None):
    if ss.reset_variables and key + '_override' in ss:
...
def build_multi_select_filter(label: str, mapping: dict[str, str], key: str, default: str, disabled: bool = False, max_selections=9999):
    selected = st.multiselect(label, mapping.keys(), key=key, default=default, disabled=disabled, max_selections=max_selections)
